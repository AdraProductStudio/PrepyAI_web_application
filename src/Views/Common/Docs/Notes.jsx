...
import {setEditNoteData, updateModalShow} from "Views/Common/Slices/Common_slice"
...
const Notes = () => {
  ...
  const handleEdit = (note_data) =>{
      dispatch(setEditNoteData({id: note_data.id, title: note_data.title, content: note_data.content}));
        ...
        {glow ? (
          <div className="w-100  row align-items-center justify-content-center mt-5">
            <div className="col-6 text-center">
              <SpinnerComponent />
              <p className="py-3">Geeting Notes</p>
            </div>
          </div>
        ) : data?.length ? (
