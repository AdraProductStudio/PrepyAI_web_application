...
import {
    update_app_data, update_error, login_reducer
...
export const handlerefreshToken = (refresh_token) => async (dispatch) => {
    ...
    try {
        ...
        else dispatch(update_error({ message: data?.message, type: "error" }))
    } catch (err) {
        dispatch(update_error({ message: err?.message, type: "error" }))
    }
}