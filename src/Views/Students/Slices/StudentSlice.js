...
const TeacherSlice = createSlice({
  ...
  reducers: {
    ...
    updateSelectedQuestionIndex: (state, action) => {
      state.mcq_test.selectedQuestionIndex =
        action.payload.selectedQuestionIndex;
    },
    updateAnswers(state, action) {
      const answeredQues = action.payload?.filter(
        (v) => v?.candidate_answer !== ""
      );
      state.mcq_test.questions = action.payload;
      state.mcq_test.answeredQuestionPercentage =
        (answeredQues?.length / action.payload?.length) * 100;
    },
    getQuestionFromDb: (state, action) => {
      ...
      state.mcq_test.questions = action.payload;
      state.mcq_test.isDataPresentInIndexedDb = action.payload?.length
        ? true
        : false;
      state.mcq_test.answeredQuestionPercentage =
        (answeredQues?.length / action.payload?.length) * 100;
