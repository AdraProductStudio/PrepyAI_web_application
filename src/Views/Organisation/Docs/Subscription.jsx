...
const Subscription = ({plans, period}) => {
  return (
    <div className='h-100 px-3 px-md-5'>
        <article className='row row-cols-1 row-cols-md-2 row-cols-xxl-4'>
            {
                plans?.map(plan => (
                    <section key={plan.type} className='px-4 pb-4 pt-md-2'>
                        <Card className={`rounded-5 px-4 py-3 shadow overflow-auto custom-scroll ${plan.active ? 'brand_color_pricing_plan' : ''}`} style={{height: "40rem"}}>
                            <Card.Body className=''>
                                ...
                                <Card.Text className='fs-1 align-items-center pb-2'>&#36;{plan.amount} <span className='fs-6'>/ per {period ? period : "month"}</span></Card.Text>
                                <ButtonComponent 
                                    buttonName={"Get Started"}
                                    className={` rounded-3 w-100 py-2 mb-3 ${plan.active ? 'btn-light' : 'btn-outline-dark'}` }
                                />
                                <hr />
                                <Card.Subtitle className='pt-3 pb-3'>Features </Card.Subtitle>
                                {
                                    plan.features.map( (feature, idx) => (
                                        <Card.Text key={idx} className='d-flex justify-content-start align-items-start gap-2'>
                                            <span>
