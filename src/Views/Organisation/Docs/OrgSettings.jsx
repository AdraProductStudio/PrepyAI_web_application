...
import { updateOrgSettingsInputs } from "../Slices/Organisation_slice";

const OrgSettings = () => {
  const { jsonOnly } = JsonData();

  const dispatch = useDispatch();
  const settingInputs = useSelector((state) => state.organisationState.settingsInputs)

  const onInputChange = (e) => {
    const { name, value } = e.target;
    dispatch(updateOrgSettingsInputs({ field: name, value }))
  };
      ...
      <Row className="justify-content-center justify-content-xl-start ms-xl-5">
        <Col xs={12} xl={7}>
          ...
          <section className="overflow-auto mt-4">
            <form className="row">
              {jsonOnly?.orgSettingsInputs.map((input, idx) => (
