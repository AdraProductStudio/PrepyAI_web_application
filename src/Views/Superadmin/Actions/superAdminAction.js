...
import { clearSettingsInputs, updateFilteredOrganizationDetails, updateFilterInputs, updateMonthlyReports, updateOrganizationDetails, updatePersonalInfoInputs, updateSubcriptionDetails } from "../Slices/SuperAdmin_slice"
...
export const createOrganization = (payload) => async (dispatch,getState) => {
  try {
    ...
    if(data?.error_code === 0){
      ...
      dispatch(update_error({ Err: data?.message || "organization creation failed", Toast_Type: "error" }))
